...
install -d -m 755 -o root -g wheel "${DESTDIR}"/export/ports/.portsnap || exit 1
...
install -c -m 600 -o guy -g guy tree/etc/local/nginx/htpasswd_guy_export "${DESTDIR}"/etc/local/nginx/htpasswd_guy_export || exit 1
